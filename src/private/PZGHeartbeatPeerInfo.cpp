...
{
   if (size < (_peerID.FlattenedSize()+sizeof(uint32))) return B_ERROR;

   const uint32 peerIDFlatSize = ZGPeerID::FlattenedSize();
   if (_peerID.Unflatten(buf, peerIDFlatSize) != B_NO_ERROR) return B_ERROR;
   ...
   if (size < (numTimings*tiFlatSize))                return B_ERROR;
   if (_timings.EnsureSize(numTimings) != B_NO_ERROR) return B_ERROR;

   for (uint32 i=0; i<numTimings; i++)
   {
      if (_timings.AddTailAndGet()->Unflatten(buf, tiFlatSize) != B_NO_ERROR) return B_ERROR;
